# Agent Instructions

## Agent skills

### Issue tracker

Issues and PRDs are tracked in GitHub Issues; external PRs are not a triage request surface. See `docs/agents/issue-tracker.md`.

### Triage labels

Use the default triage label vocabulary: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`. See `docs/agents/triage-labels.md`.

### Domain docs

This repo uses a single-context domain-doc layout: root `CONTEXT.md` plus root `docs/adr/` when they exist. See `docs/agents/domain.md`.

### Rust review

Use the `dpc-rust-general-multipart-review` skill after major Rust changes.

### Reliability context

Read root `SECURITY.md` before changing filesystem mutation, persistence,
authorization, or other trust-boundary behavior.

## Architecture context

Read root `ARCHITECTURE.md` before changing Rust CLI architecture, persistence shape, projection behavior, or command boundaries.
